...
h3. Open Source project - Buildroot & OpenWrt

p. Данные прошивки предназначены исключительно для опытных пользователей, умеющих работать с UART консолью, TFTP сервером и командами U-boot.

p. В тестовых прошивках пока нет поддержки Video, запись на FLASH не производится, старт системы происходит из памяти. Сейчас проверяется возможность загрузки ядра Linux на разных процессорах Hisilicon, работа интерфейсов Ethernet и USB, поддержка минимальной файловой системы, автоматическое детектирование сенсоров.
...
|<. Для любых устройств XM с SoC HI3518Cv2, HI3518Ev2            |=. "скачать":http://flyrouter.net/downloads/software/ipcam/Open_Source_Project/HI_SoC_v1/ |={color:orange}. в разработке                                                             |
...
p. Скачайте тестовую прошивку Buildroot или OpenWrt под ваше устройство (см. таблицу).

p. Настройте TFTP сервер и положите на него скачанный фаил, переименовав его в uImage
...
p. После загрузки устройство с прошивкой Buildroot имеет адрес 192.168.1.15

p. После загрузки устройство с прошивкой OpenWrt имеет адрес 192.168.1.10
...
p. Команды для инициализации и обновления проекта 
