---
layout:     post
title:      "Аппаратно-програмный инструментарий для изучения и ремонта устройств"
tagline:    ""
category:   ""
group:      ipcam
...
p. На данной странице будет собираться информация по аппаратным и програмным инструментам, используемым при исследовании IP-камер, видеорегистраторов и роутереов.
...
p. В терминальной программе *picocom* по команде Ctrl+A CTRL+P имеется интересная возможность подать импульс одну секунду на вывод DTR (в USB TTL UART переходник), который разорвёт цепь питания у камеры/роутера через реле (например из набора Arduino) или полевой транзистор.

p. Самые дешевые китайские USB TTL UART переходники не имеют выведенного DTR, но это, как правило не проблема - сигнал можно взять непосредственно с вывода микросхемы. Для микросхемы PL-2303HXC, это вывод *2*.
